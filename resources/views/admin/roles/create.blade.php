<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h2 class="font-semibold text-2xl text-white leading-tight">Create Role</h2>
                <p class="mt-1 text-sm text-slate-400">Add a new role with permissions.</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="rounded-2xl bg-slate-900/50 backdrop-blur-sm p-6 sm:p-8 border border-white/10">
                <form method="POST" action="{{ route('admin.roles.store') }}">
                    ...
                    <div class="mb-6">
                        ...
                        <x-text-input id="name" class="mt-1 block w-full bg-slate-800 border-slate-700 text-white placeholder-slate-400 focus:border-indigo-500 focus:ring-indigo-500" type="text" name="name" :value="old('name')" required autofocus />
                        ...
                        <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-3">
                            @foreach ($permissions as $permission)
                                <label class="flex items-center gap-3 rounded-xl bg-slate-800/50 border border-white/10 p-3 hover:bg-slate-800 transition-colors">
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" @checked(in_array($permission->name, old('permissions', []))) class="rounded border-slate-600 bg-slate-800 text-indigo-600 focus:ring-indigo-500 focus:ring-offset-slate-900">
